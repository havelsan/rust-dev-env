...
then
    ...
    DOWNLOAD_URL=$(grep $ARCH other-installation-methods.html | grep  "https://.*dist/rust-[0-9].*-$ARCH-unknown-linux-gnu.tar.xz" | sed -e 's/tar.xz.*/tar.xz/'| sed -e 's/.*"//'| grep https)
